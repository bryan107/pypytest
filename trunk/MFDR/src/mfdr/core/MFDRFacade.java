...
public class MFDRFacade {
	...
	public void learnWindowSizes(TimeSeries ts) {
		...
		// STEP 2: AYALYZE IMFs
		double windowsize_freq = wfilter.getWhiteNoiseWindowSize(imfs);
		double windowsize_trend = tfilter.getTrendWindowSize(imfs);
		this.windowsize = new WindowSize(windowsize_freq, windowsize_trend);
		
		// STEP 3: Set training result;
		mfdr.updateFrequencyWindowsize(windowsize_freq);
		mfdr.updateTrendWindowsize(windowsize_trend);
		this.windowsizetrain = true;
	}

	// PHASE 2: Learn Angle and Variations
	public void training(LinkedList<TimeSeries> ts, AngleLearning alearn,
