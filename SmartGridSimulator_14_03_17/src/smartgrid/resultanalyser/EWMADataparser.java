...
public class EWMADataparser {
	...
	public static void main(String[] args) {
		...
		for(double r = 0 ; r < 0.011 ; r += 0.001){
			...
			fagent.updatewritingpath("C:\\TEST\\result_demander_EWMA_V_" + df2.format(attackvaluedemand) + ".txt");
...
//				fagent.updatereadingpath("C:\\TEST\\GRID\\" + location +"\\DEMANDER_EWMA_V_" + df2.format(attackvaluedemand) + "\\demander_G_" + g + "_R_" + df1.format(r) + ".txt");	
		...
		for(double r = 0 ; r < 0.011 ; r += 0.001){
			DecimalFormat df1 = new DecimalFormat("0.000");
			fagent.updatewritingpath("C:\\TEST\\result_supplier_EWMA_V_" + df2.format(attackvaluesupply) + ".txt");
