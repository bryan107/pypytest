...
public class Long_AbnormalDetectionCore {
	...
	private static Log logger = LogFactory.getLog(Long_AbnormalDetectionCore.class);
	private String filewritelocation, filereadlocation, abnormaltype;

	// -----------User setup--------------

	// ------------------------------------
	public Long_AbnormalDetectionCore(String filewritelocation,
	...
	public void runSet(int num, double lowerratio, double upperratio) {
		...
		for (double faultratio = lowerratio; faultratio < upperratio + 0.00001; faultratio += 0.01) {
			...
			String writeline = "";
			writeline = "Fault Ratio:," + df.format(faultratio) + ",Total Real:," + df.format(longtermcount_real); 
			writeline = writeline + ",Total Detect:," + longtermcount_detect;
