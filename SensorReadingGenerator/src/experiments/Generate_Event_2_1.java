...
public class Generate_Event_2_1 extends TestCase {
	...
	public void test() {
		...
		for(double i = 0.007 ; i <= 0.0101; i+=0.001){
			creatSet(5, noise, i);
			creatSet(7, noise,  i);
			creatSet(10, noise,  i);
			creatSet(15, noise,  i);
	...
	private void creatSet(int number, double noise, double eventratio) {
		...
		for (int r = 0; r < 30; r++) {
			...
			f.updatewritingpath("C:\\TEST\\"+ fileaddress + eventtype.toString() + "\\source_1__"
...
//					}			
			...
			DecimalFormat df = new DecimalFormat("00.00");
			System.out.println("SYSTEM_EVENT: " + df.format((double) r * 100 / 30) + "%  processes are finished");
		}
	}
