...
public final class Calculator {
	...
	public double correlationStrengthOriginal(double input,
			double estimatecorrelation, double boundary) {
		...
		if (input < estimatecorrelation) {
			result = (input - lowerbondary)
					/ (estimatecorrelation - lowerbondary);
			if (result < 0)
				return 0;
		} else if (input > estimatecorrelation) {
			result = (upperbondary - input)
					/ (upperbondary - estimatecorrelation);
			if (result < 0)
				return 0;
		}
		return result;
	...
	public double correlationStrength(double input, double estimatecorrelation,
			double boundary) {
		double upperbondary = estimatecorrelation
				+ (estimatecorrelation * boundary);
		double lowerbondary = estimatecorrelation
				- (estimatecorrelation * boundary);
		double result = 1;
		if (input < estimatecorrelation) {
			result = (input - lowerbondary)
	...
	public double getRegressionSlope(double[] x, double[] y) {
		...
		try {
			...
			logger.error("Regression read out of boundary" + e);
			return 0;
		}
