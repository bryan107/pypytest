...
public class VariationAbnormalDetectionCore {
	...
	private static Log logger = LogFactory.getLog(VariationAbnormalDetectionCore.class);
	private String filewritelocation, filereadlocation, abnormaltype;

	// -----------User setup--------------

	// ------------------------------------
	public VariationAbnormalDetectionCore(String filewritelocation,
...
//		DFD dfd = new DFD(threshold);
		...
		while (line != null) {
			...
			ProcessedReadingPack prp= va.markReading(reading);
			Map<Integer, MarkedReading> rc = prp.markedReadingPack();
			Iterator<Integer> it = rc.keySet().iterator();
			while (it.hasNext()) {
				int nodeid = it.next();
				if (rc.get(nodeid).readingContidion() == FT) {
					totalcheck++;
