...
//	private final short UNKNOWN = 4;
	...
	private final Short devicedefaultcondition = GD;
			...
			try {
				if (deviceconditioncount.get(nodeid).size() < samplesize) {
					...
					if (devicecondition.get(nodeid) != FT) {
						...
						deviceconditioncount.get(nodeid).add(readingfaultcondition.get(nodeid));
						updateDeviceCondition(nodeid);
					}
				}
			} catch (Exception e) {
	...
	private void updateCorrelationTrendTable() {
		...
		for (int i : nodeindex) {
			for (int j : nodeindex) {
				if (j != i) {
					if(correlationmap.get(i).get(j).isEnable()){
						double correlation = correlationmap.get(i).get(j).getEstimatedCorrelation();
						correlationtrendtable.get(i).put(j, correlation);
					}
					else{
						correlationtrendtable.get(i).put(j, (reading.get(j)/reading.get(i)));
					}
				}
			}
		}
