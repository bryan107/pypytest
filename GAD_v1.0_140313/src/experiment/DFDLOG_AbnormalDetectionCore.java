...
public class DFDLOG_AbnormalDetectionCore {
	...
	private static Log logger = LogFactory.getLog(DFDLOG_AbnormalDetectionCore.class);
	private String filewritelocation, filereadlocation, abnormaltype;

	// -----------User setup--------------

	// ------------------------------------
	public DFDLOG_AbnormalDetectionCore(String filewritelocation,
...
//				+ abnormaltype + "__" + df.format(faultratio) + ".csv");
	...
	private void iterateAFile(FileAccessAgent agent) {
		...
		while (line != null) {
			...
			// Process readings
			Map<Integer, Short> rc = dfd.markReading(reading);
