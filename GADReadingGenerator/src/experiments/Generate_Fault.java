...
public class Generate_Fault extends TestCase {
	...
	public void test() {
		...
		for(double i = 0.1 ; i <= 0.6; i+=0.1){
			creatSet(4, noise, new DeviationReadingFault(), i);
			creatSet(8, noise, new DeviationReadingFault(), i);
			creatSet(16, noise, new DeviationReadingFault(), i);
	...
	private void creatSet(int number, double noise, FaultSymptom faultsymptom,
		...
		for (int r = 0; r < 30; r++) {
			...
			f.updatewritingpath("C:\\TEST\\FaultType\\" + eventtype.toString() + "\\source_1__" + faultsymptom.getKey() + "__"
					+ df2.format(faultratio) + "__NUM_" + number + "__" + r + ".cvs");
			...
			Map<Integer, LinkedList<Integer>> fault = new HashMap<Integer, LinkedList<Integer>>();
			String outputstring = "";
			for(int section = 0 ; section < round ; section++){
				LinkedList<Integer> temp = new LinkedList<Integer>();
				for(int nodeid = 0 ; nodeid < number ; nodeid++){
					if(Math.random() <= faultratio){
						temp.add(nodeid);
						outputstring = outputstring + round + ":" + nodeid + ",";
					}
				}
				fault.put(section, temp);
			}
			...
			for (int section = 0; section <= round; section++) {
				for (int nodeid = 0; nodeid < number; nodeid++) {
					if(fault.get(section).contains(nodeid)){	// at fault round node
						smanager.insertFault(nodeid, faultsymptom);
					} else{										// at normal round node
						smanager.insertFault(nodeid, new NullFault());
					}
				}
