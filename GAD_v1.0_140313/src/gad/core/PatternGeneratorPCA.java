...
public class PatternGeneratorPCA implements PatternGenerator{
	private static Log logger = LogFactory.getLog(PatternGeneratorPCA.class);
	private int windowsize;
	private final short GD = 3;
	public PatternGeneratorPCA(int windowsize) {
	...
	public Map<Integer, Map<Integer, EstimatedVariancePCA>> getEstimation(Map<Integer, Double> reading) {
		...
		while(it1.hasNext()) {
			...
			while(it2.hasNext()) {
				...
				content.put(nodeid_j, new EstimatedVariancePCA(direction, deviation, estimatedreading, true));
			}
