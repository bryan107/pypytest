...
public class GridSimulatorWithEWMA {
	...
	EWMA pmc = new EWMA();
	// Set up the threshold as 3 standard deviation.
	
	//File Access Interface
	private FileAccessAgentT fagent;
	private static Log logger = LogFactory.getLog(GridSimulatorWithEWMA.class);
	...
	public GridSimulatorWithEWMA(String writtingaddress, double averagegeneration, double averageconsumption){
...
//				logger.info("Round (" + round + "): Consumer Node ["+ key + "] is attacked");
		...
		ProcessedReadingPack prpack = pmc.markReading(readingpack);
	...
	private String runSuppliers(int totalround, int round) {
		...
		ProcessedReadingPack prpack = pms.markReading(readingpack);
		for(int i = 0 ; i < suppliercluster.size(); i++){
			if (!prpack.markedReadingPack().get(i).deviceCondition()){ //If a "permanent" attack is detected, reset node
				pms.resetNode(i);
